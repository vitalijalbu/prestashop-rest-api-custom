...
protected function getCurrencies(){
    ...
    // Try different possible locations for ObjectPresenter
    $serializer = null;
    if (class_exists('PrestaShop\PrestaShop\Adapter\ObjectPresenter')) {
        $serializer = new \PrestaShop\PrestaShop\Adapter\ObjectPresenter();
    } elseif (class_exists('ObjectPresenter')) {
        $serializer = new ObjectPresenter();
    } elseif (class_exists('PrestaShop\PrestaShop\Core\Foundation\Database\EntityInterface\ObjectPresenter')) {
        $serializer = new \PrestaShop\PrestaShop\Core\Foundation\Database\EntityInterface\ObjectPresenter();
    }
        ...
        function ($currency) use ($serializer, &$current_currency) {
            if ($serializer) {
                try {
                    ...
                    // Fallback to manual conversion if present() method fails
                    $currencyArray = [
                        'id' => $currency->id,
                        'name' => $currency->name,
